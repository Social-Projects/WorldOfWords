app.service('WordTranslationService', ["$http","$q","HttpRequest", function ($http, $q, HttpRequest) {
    ...
    this.getWordsFromInterval = function (start, end, originalLanguageId, translationLanguageId) {
        var deferred = $q.defer();
        HttpRequest.get('/api/GlobalDictionary?start=' + start + '&end=' + end +
            '&originalLangId=' + originalLanguageId + '&translationLangId=' + translationLanguageId,
            deferred);
        return deferred.promise;
    }

    this.getAmountOfWordsBySpecificLanguage = function (lang, translationLanguageId) {
        var deferred = $q.defer();
        HttpRequest.get('/api/GlobalDictionary?originalLangId=' + lang +'&translationLangId=' + translationLanguageId, deferred);
        return deferred.promise;
    }

    this.getAmountOfWordsBySearchValue = function (searchValue, lang, translationLanguageId) {
        var deferred = $q.defer();
        HttpRequest.get('/api/GlobalDictionary?searchValue=' + searchValue + '&originalLangId=' + lang + '&translationLangId=' + translationLanguageId, deferred);
        return deferred.promise;
    }

    this.getWordsBySearchValueFromInterval = function(start, end, lang, searchValue, translationLanguageId)
    {
        var deferred = $q.defer();
        HttpRequest.get('/api/GlobalDictionary?searchValue=' + searchValue + '&startOfInterval=' + start + '&endOfInterval=' + end +
            '&originalLangId=' + lang + '&translationLangId=' + translationLanguageId, deferred);
        return deferred.promise;
    }
