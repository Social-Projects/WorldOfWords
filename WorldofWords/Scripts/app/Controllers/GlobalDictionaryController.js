    ...
    function ($scope, $window, $timeout, $modal, WordTranslationService, ConstService, LanguageService, ModalService,WordsService,toastr,TagService) {
        ...
        if ($scope.searchValue.match(regExp)) {
         ...
         .then(function (response) {
             ...
             TagService.getWordsByTagValue(($scope.selectedPage - 1) * ConstService.WORDS_ON_PAGE,
            ...
            WordTranslationService.getAmountOfWordsBySpecificLanguage($scope.selectedOriginalLanguage.Id, $scope.selectedTranslationLanguage.Id)
            .then(function (response) {
                ...
                WordTranslationService.getWordsFromInterval(($scope.selectedPage - 1) * ConstService.WORDS_ON_PAGE,
                    ...
                    $scope.selectedOriginalLanguage.Id, $scope.selectedTranslationLanguage.Id)
        .then(function (response) {
            $scope.toShow = response;
        });
            });
        } else {
            WordTranslationService.getAmountOfWordsBySearchValue($scope.searchValue, $scope.selectedOriginalLanguage.Id, $scope.selectedTranslationLanguage.Id)
        .then(function (response) {
            $scope.wordsAmount = response;
            WordTranslationService.getWordsBySearchValueFromInterval(($scope.selectedPage - 1) * ConstService.WORDS_ON_PAGE,
                Math.min($scope.selectedPage * ConstService.WORDS_ON_PAGE, $scope.wordsAmount),
                $scope.selectedOriginalLanguage.Id, $scope.searchValue, $scope.selectedTranslationLanguage.Id)
            .then(function (response) {
                 $scope.toShow = response;
            });
        });
