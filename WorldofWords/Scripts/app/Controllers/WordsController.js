app.controller('WordsController', ["$scope", "$modal", "$modalInstance", "ModalService", "WordsService", 
    "LanguageService", "ConstService", "UserService", "languageId", "translationLanguageId","toastr", function ($scope, 
        $modal, $modalInstance, ModalService, WordsService,LanguageService, ConstService, UserService, 
        languageId, translationLanguageId,toastr) {
   ...
   $scope.ManipulateData = function () {
    ...
    $scope.goToEdit=function(word){
     ...
     var modalInstance = $modal.open({
            ...
            resolve: {
                ...
                bodyText: function () {
                    return 'Such word does exist. Do you want to update this word?';
                }
            }
        });
    ...
    $scope.$watch('selectedWord', function (word) {
        if (typeof (word) === 'object') {
            $scope.isDescrFieldDisabled = $scope.isTranscrFieldDisabled = $scope.isTranFieldDisabled = $scope.isSynFieldDisabled = $scope.isTagFieldDisabled = $scope.isButtonDisabled = true;
            $scope.goToEdit(word);
        }
        else {
            $scope.isDescrFieldDisabled = $scope.isTranscrFieldDisabled = $scope.isTranFieldDisabled = $scope.isSynFieldDisabled = $scope.isTagFieldDisabled = $scope.isButtonDisabled = $scope.isButtonDisabled = false;
            $scope.transcriptionOfWordToAdd = '';
            $scope.descriptionOfWordToAdd = '';
            $scope.selectedSyn = '';
            $scope.selectedTag = '';
