...
namespace WorldOfWords.Domain.Services
{
    public class WordTranslationService : IWordTranslationService
    {
        ...
        public async Task<List<WordTranslationImportModel>> GetWordsFromIntervalAsync(int startOfInterval, int endOfInterval, int originalLangId, int translationLangId)
        {
            ...
            using (var uow = unitOfWorkFactory.GetUnitOfWork())
            {
                ...
                List<WordTranslation> wordTranslations = await uow.WordTranslationRepository.GetAll()
                    .Where(item => item.OriginalWord.LanguageId == originalLangId && item.TranslationWord.LanguageId == translationLangId)
        ...
        public async Task<List<WordTranslationImportModel>> GetWordsWithSearchValueAsync(string searchValue, int startOfInterval, int endOfInterval, int originalLangId, int translationLangId)
        {
            using (var uow = unitOfWorkFactory.GetUnitOfWork())
            {
                if (startOfInterval >= uow.WordTranslationRepository.GetAll().Count() || startOfInterval < 0
        ...
        public async Task<int> GetAmountOfWordTranslationsByLanguageAsync(int originalLangId, int translationLangId)
        {
            using (var uow = unitOfWorkFactory.GetUnitOfWork())
            {
                return await uow.WordTranslationRepository.GetAll()
        ...
        public async Task<int> GetAmountOfWordsBySearchValuesAsync(string searchValue, int originalLangId, int translationLangId)
        {
            using (var uow = unitOfWorkFactory.GetUnitOfWork())
            {
                return await uow.WordTranslationRepository.GetAll()
